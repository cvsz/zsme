...
export function BankingWorkbench() {
  ...
  const viewState: LoadState | "loading" | "disconnected" | "unauthenticated" = !configuredEndpoint
    ...
    : !getAccessToken()
      ...
      : loadState === "idle"
        ? "loading"
        : loadState;
  const visibleAccounts = viewState === "disconnected" || viewState === "unauthenticated" ? [] : accounts;
  const visibleTransactions = viewState === "disconnected" || viewState === "unauthenticated" ? [] : transactions;
  const accountNames = new Map(accounts.map((account) => [account.id, account.name]));
  const unmatched = visibleTransactions.filter((transaction) => transaction.status === "unmatched");
  const reconciled = visibleTransactions.filter((transaction) => transaction.status === "reconciled");
  const incoming = visibleTransactions.filter((transaction) => Number(transaction.amount) > 0).reduce((sum, transaction) => sum + Number(transaction.amount), 0);
  const outgoing = visibleTransactions.filter((transaction) => Number(transaction.amount) < 0).reduce((sum, transaction) => sum + Math.abs(Number(transaction.amount)), 0);
  ...
  const statusLabel = {
    ...
    ready: `${visibleAccounts.length} live bank account${visibleAccounts.length === 1 ? "" : "s"}`,
    ...
    <div className="page-stack">
      <header className="page-header">
        ...
        <div className="page-actions">
          ...
          <button className="button button-primary" type="button" disabled={!canUseWorkspace || viewState !== "ready" || visibleAccounts.length === 0} onClick={() => { setActionError(""); setIsImportFormOpen(true); }}>
      ...
      <section className="metric-grid" aria-label="Banking metrics">
        <DataCard label="Active bank accounts" value={viewState === "ready" ? String(visibleAccounts.length) : "—"} meta="Organization-scoped accounts" icon={Landmark} />
